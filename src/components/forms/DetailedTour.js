...
const DetailedTour = ({ item }) => {
  ...
  const increment = () => {
    setCount(count >= item.quotaLeft ? count : count + 1);
  };
  const decrement = () => {
      ...
      <Container>
        <div className="col-10" style={{ margin: "0 auto" }}>
          ...
          <div className="d-flex justify-content-between mt-4 mb-10">
            <Image src={item.image[1]} className="image-body" />
            <Image src={item.image[2]} className="image-body" />
            <Image src={item.image[3]} className="image-body" />
          </div>
